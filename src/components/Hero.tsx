...
export default function Hero({dict,locale,catalogs,totalProducts,description}:{dict:Dictionary;locale:Locale;catalogs:Catalog[];totalProducts:number;description?:string}) {
  const ref=useRef<HTMLDivElement>(null);const {scrollYProgress}=useScroll({target:ref,offset:["start start","end start"]});const visualY=useTransform(scrollYProgress,[0,1],[0,80]);const ar=locale==="ar";const Arrow=ar?ArrowLeft:ArrowRight;const visualCatalogs=catalogs.filter(c=>c.id.startsWith("imported-")).slice(0,3);
  return <section ref={ref} className="relative min-h-[82svh] overflow-hidden border-b border-(--color-line) bg-[radial-gradient(circle_at_70%_20%,rgba(201,162,39,.13),transparent_40%)]">
    ...
    <div className="relative mx-auto grid min-h-[82svh] max-w-7xl items-center gap-12 px-5 py-20 md:px-8 lg:grid-cols-[1.05fr_.95fr] lg:py-24">
      <div className="relative z-10">
        ...
        <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:.55}} className="mt-12 flex items-center gap-8 border-t border-(--color-line) pt-6"><div><p className="text-2xl font-semibold text-(--color-ivory)">03</p><p className="mt-1 text-[10px] uppercase tracking-[.2em] text-(--color-smoke)">{ar?"مستودعات":"Warehouses"}</p></div><div className="h-9 w-px bg-(--color-line)"/><div><p className="text-2xl font-semibold text-(--color-ivory)">{catalogs.length}</p><p className="mt-1 text-[10px] uppercase tracking-[.2em] text-(--color-smoke)">{ar?"نوع حقيبة":"Bag types"}</p></div></motion.div>
      </div>
