...
export default function CatalogForm({
  ...
  const errorMessage = error === "inventory"
    ? (locale === "ar" ? "كود الموديل أو SKU أو الباركود مستخدم مسبقاً. يجب أن تكون المعرفات فريدة." : "The model code, SKU, or barcode is already in use. Identifiers must be unique.")
    : error === "slug"
  ...
  return (
    <form action={action} className="flex flex-col gap-5 rounded-2xl border border-(--color-line) bg-(--color-surface) p-6">
      ...
      <div><label className={labelClass}>{locale === "ar" ? "كود الموديل" : "Model code"}</label><input name="modelCode" required dir="ltr" placeholder="D1-16" defaultValue={catalog?.modelCode} className={inputClass}/><p className="mt-1.5 text-xs text-(--color-smoke)">{locale === "ar" ? "الكود الداخلي الثابت للموديل، وسيُستخدم لاحقاً للمطابقة مع دفترة." : "Stable internal model code used later for Daftra matching."}</p></div>
      ...
      <InventoryEditor initial={catalog?.inventory ?? catalog?.colors.map((color)=>({color,sku:"",barcode:"",quantities:{}})) ?? []} warehouses={warehouses} locale={locale}/>
